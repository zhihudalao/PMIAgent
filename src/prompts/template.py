...
def get_prompt_template(prompt_name: str, locale: str = "zh-CN") -> str:
    ...
    Args:
        ...
        locale: Language locale (e.g., en-US, zh-CN). Defaults to zh-CN
    ...
    try:
        ...
        # Try locale-specific template first (e.g., researcher.md)
        try:
            template = env.get_template(f"{prompt_name}.{normalized_locale}.md")
            return template.render()
        except TemplateNotFound:
            # Fallback to Chinese template if locale-specific not found
